...
sudo chmod +x /home/kononov/bot/bot
sudo chmod +x /home/kononov/bot/restart.sh
...
post_counter 	= 0;												//Обнуление счетчиков постов для режима #заткнись
...
while(true)															//Бесконечный цикл
{
	if (clock() - timer >= 1000000)									//Если прошла одна секнда
	{
		...
		if (minute_current != minute_previous)						//Если произошло изменение минуты
		{
			...
			for (int i = 0; i <= 8 ; i++) 
			{
				if (name_var[i][1] < 60) 	name_var[i][1] ++;		//Увеличение счетчика минут пользователя
				else {name_var[i][0] = 0; 	name_var[i][1] = 0;}	//Если прошло 60 минут, то сброс счетчика постов и счетчика минут пользователя
			}
		}		
		...
		if (Request(req_name, gus_id, arg_1, arg_2))  									//Если запрос успешен
		{
			...
			if (Json ())																//Если распарсинг успешен
			{
				...
				/**/																	//Отправка данных в main_id
				//string str_test = "";
				//str_test += to_string(hour_current) + ":" + to_string(minute_current);
				...
				else if (!flag_silence)																//Если режим #заткнись не активен
				{
					...
					if (i != 0) str = SearchSpeech(i); else str = "";								//Поиск случайной строки ответа, в соответсвующем файле
					//Ответ по ключу
					...
					{
						if (post_density < 4) 	{rand_min = 3; rand_max = 4;}						//Если за предшествующую минуту всего было меньше трех постов, то ответ на каждый 4-5 пост конкретного пользователя
						else 					{rand_min = 4; rand_max = 5;}						//Если за предшествующую минуту всего было не меньше трех постов, то ответ на каждый 7-8 пост конкретного пользователя
